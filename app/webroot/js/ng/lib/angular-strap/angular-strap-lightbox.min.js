...
angular.module('ngStrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div id='imagePreview' class='modal' tabindex='-1' role='dialog' aria-hidden='true'><div class='modal-dialog'><div class='modal-content'><div class='modal-body' ng-swipe-left='Lightbox.nextImage()' ng-swipe-right='Lightbox.prevImage()'><div class='lightbox-nav'><div class=lightbox-image-container><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}></div></div></div><div class='col-md-12 text-center' style='position: absolute;bottom: 10px;'><button class='btn btn-default' ng-click='$hide()''>ปิด</button></div></div></div>"
  );
...
angular.module('ngStrapLightbox').provider('Lightbox', function () {
  ...
  this.calculateModalDimensions = function (dimensions) {
    ...
    //        + 34px outer height of .lightbox-nav
    var height = "";
    if((dimensions.imageDisplayHeight+88) > 600){
    	height = Math.max(600, dimensions.imageDisplayHeight + 88);
    }else{
    	height = Math.min(600, dimensions.imageDisplayHeight + 88);
    }
  ...
  return {
    'link': function (scope, element, attrs) {
      ...
      var resize = function () {
        ...
        angular.element(
          document.querySelector('imagePreview.modal-dialog')
        ).css({
        ...
        angular.element(
          document.querySelector('imagePreview.modal-content')
        ).css({
